...
class PhotoLibrarySchema
{
    ...
    public function getPalette($picture, $area = null): array|null
    {
        ...
        $logger = PL_Palette_Logger::getInstance();
        ...
        try {
            ...
            // Log la création de la palette
            $processing_time = microtime(true) - $start_time;
            $area_dimensions = $area ? [
                'x' => $area['x'] ?? 0,
                'y' => $area['y'] ?? 0,
                'width' => $area['width'] ?? $image_dimensions['width'],
                'height' => $area['height'] ?? $image_dimensions['height']
            ] : null;

            $logger->log_palette_creation(
                $picture->id,
                $palette,
                $image_dimensions,
                $area_dimensions,
