...
class PL_REST_DB
{
    ...
    public function __construct(
        $wpdb,
        protected ?PhotoLibrarySchema $photoLibrarySchema = null
    ) {
        $this->wpdb = $wpdb;
    }
    ...
    protected static function initPlSchema($wpdb): PhotoLibrarySchema
    {
        try {
            ...
            $plColorHandler = new PL_COLOR_HANDLER();

            return new PhotoLibrarySchema(
                new PL_REST_DB($wpdb),
                $plColorHandler
            );
        } catch (\Exception $e) {
    ...
    public static function getRandomPicture(?int $id = 0)
    {
        ...
        $result['picture']  = self::initPlSchema($wpdb)->preparePictureDataAsArray((object)$picture);
        return $result;
    }
    ...
    public static function getPictures($offset = 0): array
    {
        try {
            ...
            $result['pictures']   = self::initPlSchema($wpdb)->prepareAllPicturesDataAsArray($result['pictures']);
    ...
    public static function getPicturesByKeywords(array $request = [], $offset = 0): array
    {
        try {
          ...
          LEFT JOIN {$wpdb->prefix}postmeta AS wp_attachment_metadata ON p.ID = wp_attachment_metadata.post_id AND wp_attachment_metadata.meta_key = '_wp_attachment_metadata'
            ...
            $photoLibrarySchema = new PhotoLibrarySchema(new PL_REST_DB($wpdb), new PL_COLOR_HANDLER());
    ...
    public function getPaletteFromDb($pictureId): array|null
    {
        $sql = $this->getWpDb()->prepare(
